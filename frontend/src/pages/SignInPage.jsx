...
const SignInPage = () => {
  const [email, setEmail] = useState("");  // Fix: Initialize state with ""
  const [password, setPassword] = useState("");
  const {signin, isLoading, error} = useAuthStore(); // Fix: Make it a state

  const handleSignIn = async (e) => {  // Fix: Correct function declaration
    e.preventDefault();
    ...
    >
      <div className="p-8">
        <h2 className='text-3xl font-bold mb-6 text-center bg-gradient-to-r
                       from-blue-500 to-blue-400 text-transparent bg-clip-text'>
          Welcome Back
        </h2>
        ...
        <form onSubmit={handleSignIn}>
          ...
          {error && <p className='text-red-500 font-semibold md-2'>{error}</p>}
          <motion.button
